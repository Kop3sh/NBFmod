...
phi = Kf .* cumsum(msg_t_resam);
s_t = A*cos((2*pi*fc*tvec_new)' + phi);
s_t = s_t';
...
env = abs(hilbert(r3_t));           %ED
r2_t = env - mean(env);             % DC blocking (cap)
figure(4);
subplot(3,1,1); plot(tvec_new,r2_t); title('envelope in t-domain');
r2_f = abs(fftshift(fft(r2_t)));
subplot(3,1,2); plot(fvec_new, r2_f); title('enevlope in f-domain');


r1_t = resample(r2_t,fs,fs_new);
% r1_f = abs(fftshift(fft(r1_t)));
% subplot(3,1,3); plot(fs, r1_f); title('output message after resampling');
sound(r1_t,fs);
